...
let monedaExtranjera = document.getElementById("moneda");

let boton = document.querySelector("button#calc");
...
boton.onclick = function() {
    if(validate()){
        let valorPesos = Number(inputPesos.value);
        let valorMoneda = Number(cotizaciones[monedaExtranjera.value]);
    
        let precioFinal = valorPesos * valorMoneda;
    
        displayResult(precioFinal);
    } else {
        alert("Flashaste wacho");
    }
...
botoncitNuevaMoneda.onclick = function () {
  if (validateNewCoin()) {
    ...
    monedaExtranjera.options.add(new Option(nombreMoneda, nombreMoneda));
